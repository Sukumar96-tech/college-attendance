...
export async function GET(request: Request) {
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "Selected branch not found or inactive.",
        },
        { status: 404 }
      );
    }
    ...
    const semesterEndExclusive = new Date(
      semester.endDate
    );

    semesterEndExclusive.setUTCDate(
      semesterEndExclusive.getUTCDate() + 1
    );
    ...
    for (const { session } of completedSessions) {
      for (const record of session.attendance) {
        ...
        const summary = studentMap.get(student.id);
    ...
    const report = {
      ...
      summary: {
        ...
        pendingAttendanceDays: Math.max(
          academicDays.length -
            completedSessions.length,
          0
        ),
    ...
    if (download === "csv") {
      ...
      return new NextResponse(csv, {
        ...
        headers: {
          ...
          "Content-Disposition": `attachment; filename="semester-attendance-${branch.code}-${studyYear.number}-semester-${semester.number}.csv"`,
